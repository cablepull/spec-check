...
export async function runDiffCheck(
  ...
  const addedByFile = parseAddedLines(patchResult.stdout);
  const files = parseNameStatus(nameResult.stdout).map<DiffFileChange>((item) => {
    const rel = item.path.replace(/\\/g, "/");
    const category = categoryFor(rel);
    categories[category].push(rel);
    return { path: rel, status: item.status, category, addedLines: addedByFile.get(rel) ?? [] };
  });
    ...
    for (const line of file.addedLines) {
      for (const trigger of ADR_TRIGGER_PATTERNS) {
        if (trigger.pattern.test(line)) {
          triggerSignals.push(trigger.kind);
          triggerEvidence.push(`${file.path}: ${line.trim()}`);
        }
      }
    }
  }
  const depAdded = files
    .filter((file) => file.category === "dependencies")
    .flatMap((file) => dependencyAdditions(file));

  if ((depAdded.length > 0 || triggerSignals.length > 0) && !adrChanged) {
    criteria.push({
      id: "R-27",
      status: "VIOLATION",
      detail: "Diff introduced dependency or architectural trigger signals without an ADR change.",
      evidence: [
        ...depAdded.map((dep) => `dependency: ${dep}`),
        ...triggerEvidence.slice(0, 10),
      ],
      fix: "Add or update an ADR in the same diff to record the architectural decision.",
    });
  } else if (depAdded.length > 0 || triggerSignals.length > 0) {
    criteria.push({
      id: "R-27",
      status: "PASS",
      detail: "ADR trigger signals were accompanied by an ADR change.",
    });
  }

  const invalidatedAssumptionFiles = files.filter((file) => file.addedLines.some((line) => /\binvalidated\b/i.test(line)));
  if (invalidatedAssumptionFiles.length > 0) {
